...
(function (factory) {
    ...
    extend(Orienter.prototype, {
        ...
        init: function () {
            this._orient = this.orientHandler.bind(this);//定义时的this要是实例化之后的this
            window.addEventListener('deviceorientation', this._orient, false);//直接this.orientHandler不行吗?
            //this.orientHandler = this.orientHandler.bind(this);
            //window.addEventListener('deviceorientation', this.orientHandler,false);//这一句里的this本应是window,用bind把this为实例给传进去了。

            this._change = this.changeHandler.bind(this);
            window.addEventListener('orientationchange', this._change, false);
            //window.addEventListener('orientationchange',this.changeHandler,false);
        },
